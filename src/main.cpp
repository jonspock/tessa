...
void CheckForkWarningConditions() {
  ...
  // of our head, drop it
  if (pindexBestForkTip && chainActive.Height() - pindexBestForkTip->nHeight >= 72) pindexBestForkTip = NULL;
...
bool static DisconnectTip(CValidationState& state) {
  ...
  for (const CTransaction& tx : block.vtx) { SyncWithWallets(tx, NULL); }
  return true;
}
...
bool static ConnectTip(CValidationState& state, CBlockIndex* pindexNew, CBlock* pblock, bool fAlreadyChecked) {
  ...
  if (pblock == NULL) fAlreadyChecked = false;
  ...
  // to conflicted:
  for (const CTransaction& tx : txConflicted) { SyncWithWallets(tx, NULL); }
...
CBlockIndex* AddToBlockIndex(const CBlock& block) {
  ...
  if (pindexBestHeader == NULL || pindexBestHeader->nChainWork < pindexNew->nChainWork) pindexBestHeader = pindexNew;
    ...
    while (!blkdat.eof()) {
      ...
      try {
        ...
        if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
          CValidationState state;
          if (ProcessNewBlock(state, NULL, &block, dbp)) nLoaded++;
...
void static CheckBlockIndex() {
  ...
  while (pindex != NULL) {
    ...
    // being set.
    assert((pindexFirstMissing != NULL) ==
           (pindex->nChainTx ==
    ...
    if (!CBlockIndexWorkComparator()(pindex, chainActive.Tip()) && pindexFirstMissing == NULL) {
      if (pindexFirstInvalid == NULL) {  // If this block sorts at least as good as the current tip and is valid, it
    ...
    if (pindex->pprev && pindex->nStatus & BLOCK_HAVE_DATA && pindexFirstMissing != NULL) {
      if (pindexFirstInvalid == NULL) {  // If this block has block data available, some parent doesn't, and has no
...
bool ProcessMessages(CNode* pfrom) {
  ...
  while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
    ...
    try {
      ...
      PrintExceptionContinue(&e, "ProcessMessages()");
    } catch (...) { PrintExceptionContinue(NULL, "ProcessMessages()"); }
