...
int GetChecksumHeight(uint32_t nChecksum, CoinDenomination denomination) {
  ...
  while (pindex) {
    ...
    if (pindex->nHeight % 10 == 0) {
      if (pindex->nHeight + 10 > chainActive.Height()) return 0;
...
bool GenerateAccumulatorWitness(const PublicCoin& coin, Accumulator& accumulator, AccumulatorWitness& witness,
  ...
  if (pindexCheckpoint) nHeightStop = pindexCheckpoint->nHeight - 10;
  ...
  while (pindex) {
    ...
    if (pindex->nHeight >= nHeightStop || fSecurityLevelSatisfied) {
      // If this height is within the invalid range (when fraudulent coins were being minted), then continue past this
      // range
      if (InvalidCheckpointRange(pindex->nHeight)) continue;
