...
template <unsigned int BITS> class base_uint {
 ...
 public:
  ...
  uint64_t Get64(int n = 0) const { return pn[2 * n] | (uint64_t)pn[2 * n + 1] << 32; }
  ...
  unsigned int GetSerializeSize() const { return sizeof(pn); }

  template <typename Stream> void Serialize(Stream& s) const { s.write((char*)pn, sizeof(pn)); }

  template <typename Stream> void Unserialize(Stream& s) { s.read((char*)pn, sizeof(pn)); }
...
class uint512 : public base_uint<512> {
 public:
  ...
  uint256 trim256() const {
    uint256 ret;
    for (unsigned int i = 0; i < uint256::WIDTH; i++) { ret.pn[i] = pn[i]; }
    return ret;
  }
};
