...
CZkpWallet::CZkpWallet(std::string strWalletFile)
{
        ...
        if (walletdb.ReadZKPSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZKPSeed_deprecated()) {
                    LogPrintf("%s: Updated zZZZ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CZkpWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
...
void CZkpWallet::Lock()
{
    seedMaster = 0;
}

void CZkpWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CZkpWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
bool CZkpWallet::LoadMintPoolFromDB()
{
...
void CZkpWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CZkpWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CZkpWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CZkpWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
...
void CZkpWallet::SeedToZKP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
void CZkpWallet::GenerateDeterministicZKP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CZkpWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CZkpWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
