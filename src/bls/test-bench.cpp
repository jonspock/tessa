...
void benchSigs() {
  ...
  for (size_t i = 0; i < numIters; i++) { sk.Sign(message1, sizeof(message1)); }
...
void benchAggregateSigsSecure() {
  ...
  endStopwatch("Generate aggregate signature, same message", start, numIters);
...
void benchAggregateSigsSimple() {
  ...
  auto start = startStopwatch();
  Signature aggSig = BLS::AggregateSigs(sigs);
  endStopwatch("Generate aggregate signature, distinct messages", start, numIters);

  auto start2 = startStopwatch();
  ASSERT(BLS::Verify(aggSig));
  endStopwatch("Verify aggregate signature, distinct messages", start2, numIters);
}
...
void benchDegenerateTree() {
  ...
  endStopwatch("Generate degenerate aggSig tree", start, numIters);

  start = startStopwatch();
  ASSERT(BLS::Verify(aggSig));
  endStopwatch("Verify degenerate aggSig tree", start, numIters);
}
