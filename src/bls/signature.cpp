...
const AggregationInfo *Signature::GetAggregationInfo() const { return &aggregationInfo; }

void Signature::SetAggregationInfo(const AggregationInfo &newAggregationInfo) { aggregationInfo = newAggregationInfo; }
...
Signature Signature::DivideBy(std::vector<Signature> const &divisorSigs) const {
  ...
  for (const Signature &divisorSig : divisorSigs) {
    std::vector<CPubKey> pks = divisorSig.GetAggregationInfo()->GetPubKeys();
    std::vector<uint8_t *> messageHashes = divisorSig.GetAggregationInfo()->GetMessageHashes();
    if (pks.size() != messageHashes.size()) { throw string("Invalid aggregation info."); }
    relic::bn_t quotient;
    for (size_t i = 0; i < pks.size(); i++) {
      relic::bn_t divisor;
      bn_new(divisor);
      divisorSig.GetAggregationInfo()->GetExponent(&divisor, messageHashes[i], pks[i]);
      relic::bn_t dividend;
      bn_new(dividend);
      try {
        aggregationInfo.GetExponent(&dividend, messageHashes[i], pks[i]);
      } catch (std::out_of_range e) { throw string("Signature is not a subset."); }
...
const uint8_t &Signature::operator[](size_t pos) const { return data[pos]; }

const uint8_t *Signature::begin() const { return data; }

const uint8_t *Signature::end() const { return data + size(); }

size_t Signature::size() const { return Signature::SIGNATURE_SIZE; }
...
bool operator!=(Signature const &a, Signature const &b) { return !(a == b); }
