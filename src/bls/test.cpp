...
TEST_CASE("Util tests") {
  ...
  SECTION("Should calculate public key fingerprints") {
    ...
    ExtendedPrivateKey esk = ExtendedPrivateKey::FromSeed(seed, sizeof(seed));
...
TEST_CASE("Signatures") {
  SECTION("Should sign and verify") {
    ...
    sig1.SetAggregationInfo(AggregationInfo::FromMsg(pk1, message1, sizeof(message1)));
    REQUIRE(BLS::Verify(sig1));

    uint8_t hash[32];
    BLSUtil::Hash256(hash, message1, 7);
    Signature sig2 = sk1.SignPrehashed(hash);
    sig2.SetAggregationInfo(AggregationInfo::FromMsg(pk1, message1, sizeof(message1)));
    REQUIRE(sig1 == sig2);
  ...
  SECTION("Should serialize and deserialize") {
    ...
    uint8_t* skData = BLSUtil::SecAlloc<uint8_t>(Signature::SIGNATURE_SIZE);
    ...
    REQUIRE(sig1 == sig2);
    sig2.SetAggregationInfo(AggregationInfo::FromMsg(pk2, message1, sizeof(message1)));
  ...
  SECTION("Should aggregate same message") {
    ...
    aggSig.SetAggregationInfo(AggregationInfo::FromMsg(aggPubKey, message1, sizeof(message1)));
    REQUIRE(BLS::Verify(aggSig));
  }
  ...
  SECTION("Should perform batch verification") {
    ...
    std::vector<Signature> const sigs = {sig1, sig2, sig3, sig4, sig5, sig6, sig7};
    std::vector<CPubKey> const pubKeys = {pk1, pk2, pk3, pk4, pk3, pk2, pk4};
    std::vector<uint8_t*> const messages = {message1, message1, message2, message3, message1, message1, message2};
    std::vector<size_t> const messageLens = {sizeof(message1), sizeof(message1), sizeof(message2), sizeof(message3),
  ...
  SECTION("Should aggregate same message with agg sk") {
    ...
    const CPrivKey aggSk = BLS::AggregatePrivKeys(privateKeys, pubKeys, true);
...
TEST_CASE("HD keys") {
  ...
  SECTION("Should match derivation through private and public keys") {
    ...
    CPrivKey sk3 = esk.PrivateChild(0).PrivateChild(3).PrivateChild(8).PrivateChild(1).GetPrivateKey();

    CPubKey pk4 = epk.PublicChild(0).PublicChild(3).PublicChild(8).PublicChild(1).GetPublicKey();
  ...
  SECTION("Should serialize extended keys") {
    ...
    ExtendedPrivateKey sk3 = esk.PrivateChild(0).PrivateChild(3).PrivateChild(8).PrivateChild(1);

    ExtendedPublicKey pk4 = epk.PublicChild(0).PublicChild(3).PublicChild(8).PublicChild(1);
    ...
    REQUIRE(std::memcmp(buffer2, buffer3, ExtendedPublicKey::EXTENDED_PUBLIC_KEY_SIZE) == 0);
  }
}
...
TEST_CASE("AggregationInfo") {
  ...
  SECTION("Should aggregate with multiple levels, different messages") {
    ...
    std::vector<size_t> const messageLensL = {sizeof(message1), sizeof(message2)};
    const Signature aggSigL = BLS::AggregateSigs(sigsL);
    ...
    std::vector<size_t> const messageLensR = {sizeof(message3), sizeof(message4)};
    const Signature aggSigR = BLS::AggregateSigs(sigsR);
    ...
    std::vector<uint8_t*> allMessages = {message1, message2, message3, message4};
    std::vector<size_t> allMessageLens = {sizeof(message1), sizeof(message2), sizeof(message3), sizeof(message4)};
  ...
  SECTION("README") {
    ...
    uint8_t seed[] = {0,   50, 6,   244, 24,  199, 1,  25, 52, 88,  192, 19, 18, 12, 89,  6,
    ...
    const CPrivKey aggSk = BLS::AggregatePrivKeys(privateKeysList, pubKeysList, true);
