...
bool PaymentRequestPlus::SerializeToString(string* output) const { return paymentRequest.SerializeToString(output); }

bool PaymentRequestPlus::IsInitialized() const { return paymentRequest.IsInitialized(); }
...
bool PaymentRequestPlus::getMerchant(X509_STORE* certStore, QString& merchant) const {
  ...
  for (int i = certs.size() - 1; i > 0; i--) { sk_X509_push(chain, certs[i]); }
  ...
  for (unsigned int i = 0; i < certs.size(); i++) X509_free(certs[i]);
