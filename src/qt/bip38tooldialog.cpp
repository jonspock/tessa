...
Bip38ToolDialog::Bip38ToolDialog(QWidget* parent) : QDialog(parent), ui(new Ui::Bip38ToolDialog), model(0) {
  ui->setupUi(this);
...
Bip38ToolDialog::~Bip38ToolDialog() { delete ui; }

void Bip38ToolDialog::setModel(WalletModel* model) { this->model = model; }
...
void Bip38ToolDialog::on_addressBookButton_ENC_clicked() {
  if (model && model->getAddressTableModel()) {
    ...
    if (dlg.exec()) { setAddress_ENC(dlg.getReturnValue()); }
  }
}

void Bip38ToolDialog::on_pasteButton_ENC_clicked() { setAddress_ENC(QApplication::clipboard()->text()); }
...
bool isValidPassphrase(QString strPassphrase, QString& strInvalid) {
  for (int i = 0; i < strPassphrase.size(); i++) {
    if (!validChar.contains(strPassphrase[i], Qt::CaseSensitive)) {
      if (QString("\"'").contains(strPassphrase[i])) continue;
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked() {
  ...
  if (!isValidPassphrase(qstrPassphrase, strInvalid)) {
    ...
    ui->statusLabel_ENC->setText(tr("The entered passphrase is invalid. ") + strInvalid + QString(" is not valid") +
                                 QString(" ") + tr("Allowed: 0-9,a-z,A-Z,") + specialChar);
    return;
  }
  ...
  if (!addr.IsValid()) {
    ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
    ui->statusLabel_ENC->setText(tr("The entered address is invalid.") + QString(" ") +
  ...
  std::string encryptedKey =
      BIP38_Encrypt(addr.ToString(), qstrPassphrase.toStdString(), key.GetPrivKey_256(), key.IsCompressed());
  ui->encryptedKeyOut_ENC->setText(QString::fromStdString(encryptedKey));
}

void Bip38ToolDialog::on_copyKeyButton_ENC_clicked() { GUIUtil::setClipboard(ui->encryptedKeyOut_ENC->text()); }
...
void Bip38ToolDialog::on_decryptKeyButton_DEC_clicked() {
  ...
  if (!BIP38_Decrypt(strPassphrase, strKey, privKey, fCompressed)) {
    ...
    ui->statusLabel_DEC->setText(tr("Failed to decrypt.") + QString(" ") +
                                 tr("Please check the key and passphrase and try again."));
    return;
  }
