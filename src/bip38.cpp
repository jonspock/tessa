...
void ComputePreFactor(std::string strPassphrase, std::string strSalt, uint256& prefactor) {
  ...
  uint64_t s = uint256S(ReverseEndianString(strSalt)).GetLow64();
  scrypt_hash(strPassphrase.c_str(), strPassphrase.size(), BEGIN(s), strSalt.size() / 2, BEGIN(prefactor), 16384, 8, 8,
              32);
}
...
bool ComputePasspoint(uint256 passfactor, CPubKey& passpoint) {
  ...
  if (passpoint.size() != clen)
    if (passpoint.size() != clen) return false;
  return false;
}
...
void ComputeSeedBPass(CPubKey passpoint, std::string strAddressHash, std::string strOwnerSalt, uint512& seedBPass) {
  ...
  scrypt_hash(BEGIN(passpoint), HexStr(passpoint).size() / 2, BEGIN(s2), salt.size() / 2, BEGIN(seedBPass), 1024, 1, 1,
              64);
}
...
std::string BIP38_Encrypt(std::string strAddress, std::string strPassphrase, uint256 privKey, bool fCompressed) {
  ...
  uint512 hashed;
  uint64_t salt = uint256S(ReverseEndianString(strAddressHash)).GetLow64();
  scrypt_hash(strPassphrase.c_str(), strPassphrase.size(), BEGIN(salt), strAddressHash.size() / 2, BEGIN(hashed), 16384,
              8, 8, 64);
  ...
  uint256 block1 = ArithToUint256((UintToArith256(privKey) << 128) ^ (UintToArith256(derivedHalf1) << 128) >> 128);
  ...
  arith_uint256 dh12 = UintToArith256(derivedHalf1) >> 128;
  ...
  uint512 eKey(uint512S(ReverseEndianString(strPrefix + strAddressHash)));
  arith_uint512 encryptedKey = UintToArith512(eKey);

  // add encrypted1 to the end of encryptedKey
  encryptedKey = encryptedKey | (UintToArith512(encrypted1) << 56);

  // add encrypted2 to the end of encryptedKey
  encryptedKey = encryptedKey | (UintToArith512(encrypted2) << (56 + 128));
  ...
  uint512 b58Checksum(uint512S(hashChecksum.ToString().substr(64 - 8, 8)));
...
bool BIP38_Decrypt(std::string strPassphrase, std::string strEncryptedKey, uint256& privKey, bool& fCompressed) {
  ...
  if (strKey.size() != (78 + 8)) return false;

  // invalid prefix
  if (uint256S(ReverseEndianString(strKey.substr(0, 2))) != uint256S("0x01")) return false;

  uint256 type(uint256S(ReverseEndianString(strKey.substr(2, 2))));
  uint256 flag(uint256S(ReverseEndianString(strKey.substr(4, 2))));
  ...
  fCompressed = (UintToArith256(flag) & UintToArith256(uint256S("0x20"))) != 0;

  // not ec multiplied
  if (type == uint256S("0x42")) {
    uint512 hashed;
    ...
    return true;
  } else if (type != uint256S("0x43"))  // invalid type
    return false;

  bool fLotSequence = (UintToArith256(flag) & 0x04) != 0;

  std::string prefactorSalt = ownersalt;
  if (fLotSequence) prefactorSalt = ownersalt.substr(0, 8);
  ...
  CPubKey passpoint;
  if (!ComputePasspoint(passfactor, passpoint)) return false;
  ...
  /** Decrypt encryptedpart2 using AES256Decrypt to yield the last 8 bytes of seedb and the last 8 bytes of
   * encryptedpart1. **/
  uint256 decryptedPart2;
  DecryptAES(encryptedPart2, derivedHalf2, decryptedPart2);
  ...
  uint256 decryptedPart1;
  arith_uint256 x2 = x1 & UintToArith256(uint256S("0xffffffffffffffff"));  // set x2 to seedbPart1 (still encrypted)
  ...
  arith_uint256 x3 = UintToArith256(derivedHalf1) & UintToArith256(uint256S("0xffffffffffffffffffffffffffffffff"));
  arith_uint256 seedbPart1 = UintToArith256(decryptedPart1) ^ x3;
  arith_uint256 seedB = seedbPart1 | (seedbPart2 << 128);

  arith_uint256 factorB;
