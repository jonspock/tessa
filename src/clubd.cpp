 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Club (http://www.club.org),
...
bool AppInit(int argc, char* argv[])
{
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "club:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in clubd anymore. Use the club-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
