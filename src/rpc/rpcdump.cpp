...
UniValue bip38encrypt(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() != 2)
    throw runtime_error(
        "bip38encrypt \"clubaddress\" \"passphrase\"\n"
        "\nEncrypts a private key corresponding to 'clubaddress'.\n" +
        HelpRequiringPassphrase() +
        "\n"

        "\nArguments:\n"
        "1. \"clubaddress\"   (string, required) The club address for the private key (you must hold the key already)\n"
        "2. \"passphrase\"   (string, required) The passphrase you want the private key to be encrypted with - Valid "
        "special chars: !#$%&'()*+,-./:;<=>?`{|}~ \n"
...
UniValue bip38decrypt(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() != 2)
    throw runtime_error(
        "bip38decrypt \"clubaddress\" \"passphrase\"\n"
  ...
  if (!key.IsValid()) throw JSONRPCError(RPC_WALLET_ERROR, "Private Key Not Valid");
  ...
  CKeyID vchAddress = pubkey.GetID();
  {
    ...
    // Don't throw error in case a key is already there
    if (pwalletMain->HaveKey(vchAddress)) throw JSONRPCError(RPC_WALLET_ERROR, "Key already held by wallet");
