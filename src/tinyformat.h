...
class FormatIterator {
 public:
  ...
  void finish() {
    ...
    if (*m_fmt != '\0') TINYFORMAT_ERROR("tinyformat: Too many conversion specifiers in format string");
  }
...
#define TINYFORMAT_DEFINE_FORMAT_C_STRING_TRUNCATE(type)                                        \
  static bool formatCStringTruncate(std::ostream &out, type *value, std::streamsize truncLen) { \
    ...
    while (len < truncLen && value[len] != 0) ++len;                                            \
    out.write(value, len);                                                                      \
    return true;                                                                                \
  }
...
#undef TINYFORMAT_DEFINE_FORMAT_C_STRING_TRUNCATE
  ...
  static const char *streamStateFromFormat(std::ostream &out, unsigned int &extraFlags, const char *fmtStart,
...
TINYFORMAT_NOINLINE  // < greatly reduces bloat in optimized builds
  ...
  if (m_extraFlags & (Flag_VariableWidth | Flag_VariablePrecision)) {
    ...
    fmtEnd = streamStateFromFormat(m_out, m_extraFlags, m_fmt, m_variableWidth, m_variablePrecision);
  }
  ...
  if (!(m_extraFlags & (Flag_SpacePadPositive | Flag_TruncateToPrecision)))
    ...
    if (m_extraFlags & Flag_SpacePadPositive) tmpStream.setf(std::ios::showpos);
...
inline const char *FormatIterator::streamStateFromFormat(std::ostream &out, unsigned int &extraFlags,
                                                         const char *fmtStart, int variableWidth,
  ...
  for (;; ++c) {
    switch (*c) {
      ...
      case ' ':
        ...
        if (!(out.flags() & std::ios::showpos)) extraFlags |= Flag_SpacePadPositive;
        continue;
      case '+':
  ...
  switch (*c) {
    ...
    case 'c':
      ...
      break;
    case 's':
      if (precisionSet) extraFlags |= Flag_TruncateToPrecision;
...
template <typename T1> void format(FormatIterator &fmtIter, const T1 &value1) {
  fmtIter.accept(value1);
  fmtIter.finish();
}

// General version for C++11
template <typename T1, typename... Args> void format(FormatIterator &fmtIter, const T1 &value1, const Args &... args) {
  fmtIter.accept(value1);
  format(fmtIter, args...);
}
...
inline void format(FormatIterator &fmtIter) { fmtIter.finish(); }
...
template <typename T1, typename... Args> std::string format(const char *fmt, const T1 &v1, const Args &... args) {
...
template <typename T1, typename... Args> void printf(const char *fmt, const T1 &v1, const Args &... args) {
...
#define TINYFORMAT_WRAP_FORMAT_N(n, returnType, funcName, funcDeclSuffix, bodyPrefix, streamName, bodySuffix)    \
  ...
  returnType funcName(TINYFORMAT_WRAP_FORMAT_EXTRA_ARGS const char *fmt, TINYFORMAT_VARARGS(n)) funcDeclSuffix { \
