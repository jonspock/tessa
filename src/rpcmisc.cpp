...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zkpObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are club addresses or hex-encoded public keys\n"
            "     [\n"
