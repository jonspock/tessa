...
struct UISignals {
  boost::signals2::signal<CClientUIInterface::ThreadSafeMessageBoxSig, boost::signals2::last_value<bool> >
      ThreadSafeMessageBox;
  //    boost::signals2::signal<CClientUIInterface::ThreadSafeQuestionSig, boost::signals2::last_value<bool>>
  //    ThreadSafeQuestion;
  ...
  boost::signals2::signal<CClientUIInterface::NotifyBlockSizeSig> NotifyBlockSize;
...
bool CClientUIInterface::ThreadSafeMessageBox(const std::string& message, const std::string& caption, uint32_t style) {
...
// void CClientUIInterface::NotifyNetworkActiveChanged(bool networkActive) { return
// g_ui_signals.NotifyNetworkActiveChanged(networkActive); } void CClientUIInterface::NotifyAlertChanged() { return
// g_ui_signals.NotifyAlertChanged(); }
...
void CClientUIInterface::ShowProgress(const std::string& title, int nProgress) {
  return g_ui_signals.ShowProgress(title, nProgress);
}
void CClientUIInterface::NotifyBlockTip(const uint256& i) { return g_ui_signals.NotifyBlockTip(i); }
...
std::string AmountHighWarn(const std::string& optname) { return strprintf(_("%s is set very high!"), optname); }
