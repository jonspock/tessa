...
UniValue importzerocoins(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() == 0)
    throw runtime_error(
        ...
        "\n[{\"d\":denomination,\"p\":\"pubcoin_hex\",\"s\":\"serial_hex\",\"r\":\"randomness_hex\",\"t\":\"txid\","
        "\"h\":height, \"u\":used},{\"d\":...}]\n"
        ...
        HelpExampleCli("importzerocoins",
                       "\'[{\"d\":100,\"p\":\"mypubcoin\",\"s\":\"myserial\",\"r\":\"randomness_hex\",\"t\":\"mytxid\","
                       "\"h\":104923, \"u\":false},{\"d\":5,...}]\'") +
  ...
  for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
    ...
    if (!vDenom.isNum()) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing d key");
    int d = vDenom.get_int();
    if (d < 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, d must be positive");
    ...
    if (vVersion.isNum()) nVersion = static_cast<uint8_t>(vVersion.get_int());
