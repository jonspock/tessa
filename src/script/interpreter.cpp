...
bool static IsCompressedOrUncompressedPubKey(const valtype& vchPubKey) {
    // No notion of Compressed/Uncompressed, maybe add more checks later TBD
   return (vchPubKey.size() == 48);
}
...
bool static IsValidSignatureEncoding(const std::vector<uint8_t>& sig) {
    // Can add more checks later : HACK TBD
    return (sig.size() == 97);
}
...
bool static IsLowDERSignature(const valtype& vchSig, ScriptError* serror) {
  ...
  //     follows the actual signature data.
    ...
    // TBD remove CheckLowS - not appropriate
